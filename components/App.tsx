...
const App: React.FC = () => {
  ...
  const loadUserData = async (user: User) => {
      // SECURITY FIX: Strictly verify user exists in DB. Do not fallback to session object.
      // This prevents authentication bypass via session storage injection.
      const currentUser = await db.getItem<User>(`basis_user_${user.id}`);

      if (!currentUser) {
          console.warn("Security: User record not found in database. Session invalid.");
          handleLogout();
          return;
      }
